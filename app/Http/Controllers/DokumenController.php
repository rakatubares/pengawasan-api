...
class DokumenController extends Controller
{
	...
	private $tanggal;
	private $tahun;
	private $unpublished_status = [100, 101];
	...
	private function updateDocNumberAndDate($model, $doc_id, $number, $jenis_surat)
	{
		...
		$update_result = $model::where('id', $doc_id)
			->update([
				'no_dok' => $number,
				'thn_dok' => $this->tahun,
				'no_dok_lengkap' => $jenis_surat . '-' . $number . $agenda_dok . $this->tahun,
	...
	public function deleteDocument($model, $doc_id)
	{
		...
		if ($is_unpublished) {
			...
			$update_result = $model::where('id', $doc_id)->update(['kode_status' => 300]); // Update kode status
			$delete_result = $model::where('id', $doc_id)->delete(); // Delete data

			// Rollback if either transaction failed
			if ($update_result == 0 || $delete_result == 0) {
				DB::rollBack();
